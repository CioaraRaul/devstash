...
- 2026-03-25: Auth Setup - NextAuth + GitHub Provider completed
  - NextAuth v5 (beta) with Prisma adapter and JWT session strategy
  - Split auth config pattern (auth.config.ts + auth.ts) for edge compatibility
  - GitHub OAuth provider
  - Proxy-based route protection for /dashboard/* (src/proxy.ts)
